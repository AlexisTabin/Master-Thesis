...
% In the context of autonomous driving, the car relies, among other technologies, on a classifier that examines a picture of a road sign and determines whether to stop, turn left, or slow down. However, it can be problematic if minor modifications to the road sign, such as stickers or insects, completely alter the classifier's output and cause the car to speed up unexpectedly. These slight modifications are known as Adversarial Perturbations \cite{moosavi_dezfooli_universal_2017} on the model. The perturbations slightly alter many pixels in an image to deceive the model. The changes are invisible to humans and, therefore, not interpretable.
...
%  This approach is promising, but as a perturbation tends to change every pixel in the image by a tiny margin to fool the model, the explanation is often not humanly interpretable. 
% While Adversarial Perturbations focus on imperceptible changes to input data for image classification tasks, \gls{cfe} methods strive to alter the fewest variables necessary to achieve user-interpretable solutions. \\
